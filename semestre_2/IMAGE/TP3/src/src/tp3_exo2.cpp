...
cv::Mat computeCumulatedHistogramGS(const cv::Mat& imageGS)
{
  ...
  // create and initialize and 1d array of integers
  cv::Mat res(256,1,CV_32S,cv::Scalar(0));

  int sum = 0;
  // compute the histogram
  for(int i=0; i<256; ++i)
  {
	sum += histogram.at<int>(i);
    	res.at<int>(i) = sum;
  }
...
{
  cv::Mat histogram = computeCumulatedHistogramGS(imageGS);
  // create a LUT
  cv::Mat myLUT(256,1,CV_8U);

  // find min and max value
  double minValue,maxValue;
  cv::minMaxLoc(imageGS, &minValue, &maxValue);

  // compute the normalized image LUT
  for(int i=0; i < 256; i++)
    myLUT.at<unsigned char>(i) = clamp_uchar(255 * histogram.at<int>(i) / (imageGS.rows * imageGS.cols)); 
...
int main(int argc, char ** argv)
{
  ...
  cv::imshow("histogramme", histogramToImageGS(computeCumulatedHistogramGS(image)));
  cv::waitKey();
